...
- (NSString *)formatLogMessage:(DDLogMessage *)logMessage
{
    ...
    if ((logMessage->logLevel & LOG_FLAG_WARN) ==LOG_FLAG_WARN)
    {
        ...
        return [NSString stringWithFormat:@"%@: %@ | %s @ %i | %@",
                ...
                [self formatXML:logMessage->logMsg]];
    }
    else
    {
...
-(NSString *)formatXML:(NSString *)logMessage
{
    //    <root>
    //        <child>
    //            <child>
    ...
    //        </child>
    //    </root>
    NSString *msg = [logMessage stringByReplacingOccurrencesOfString:@"<" withString:@"\r\n<"];
    ...
    for (NSString *chunk in chunks) 
    {
        if ([chunk hasPrefix:@"</"])
        {
            tabified =[NSString stringWithFormat:@"%@\r\n%@",tabified,[self tabify:chunk tabLevel:tabLevel]];
            tabLevel--;
        }
...
-(NSString *)tabify:(NSString*)message tabLevel:(int) level
{
    NSString *tabified=message;
    for (int i=0; i<level; i++) 
    {
        tabified =[NSString stringWithFormat:@"\t%@",tabified];
    }
    return tabified;
}
